...
After the user applied the combined migration on 14 September 2026, the live read-only schema check passed for all expected tables and columns, the academic and event catalogs, and the registration-count function. A schema check does not prove live row-level security, storage policies or authentication flows are correct.
...
2. Apply `supabase/migrations/202609130001_account_approval.sql` to a test project first, then verify it before applying it to the configured project. It adds approval/push fields and notification deduplication, protects role/status updates, and provides admin event permissions. It preserves existing account statuses; if the column is absent, existing profiles default to approved for compatibility.
   Then apply `supabase/migrations/202609140001_mobile_data.sql` for the mobile reference catalogs and visibility-checked registration totals. Both migrations are packaged in `supabase/APPLY_MOBILE_UPDATES.sql`; see [mobile database setup](MOBILE_DATABASE_SETUP.md).
