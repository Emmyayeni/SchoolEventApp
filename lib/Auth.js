...
function normalizeProfileInput(profile = {}) {
  ...
  const roleLevel =
    accountType === "staff" ? profile.roleDesignation : profile.level;
...
function mapToAppUser(authUser, profileRow = null) {
  ...
  return {
    ...
    matricNumber:
      profileRow?.matric_number ||
      profileRow?.matricNumber ||
      metadata.matricNumber ||
      "",
    staffId:
      profileRow?.staff_id || profileRow?.staffId || metadata.staffId || "",
    roleDesignation:
      profileRow?.role_designation ||
      profileRow?.roleDesignation ||
      metadata.roleDesignation ||
      "",
    phoneNumber:
      profileRow?.phone_number || metadata.phoneNumber || metadata.phone || "",
    avatar: resolveStoragePublicUrl(
      avatarValue,
      STORAGE_BUCKETS.avatars,
      DEFAULT_AVATAR,
    ),
    themeMode: normalizeThemeMode(
      profileRow?.theme_mode ||
        profileRow?.themeMode ||
        metadata.themeMode ||
        metadata.theme_mode,
    ),
  };
...
async function writeProfileRow(
  ...
  const status =
    normalized.accountType === "staff" || normalized.accountType === "organizer"
      ? "pending"
      : "approved";
...
async function ensureProfileRow(authUser, existingProfileRow) {
  ...
  try {
    await upsertProfile(
      authUser.id,
      authUser.email,
      profileInputFromAuthUser(authUser),
    );
    return await fetchProfile(authUser.id);
  } catch (error) {
    console.log(
      "Profile bootstrap on sign-in failed:",
      error?.message || error,
    );
    throw error;
  }
...
export async function signInWithEmailPassword({ email, password }) {
  ...
  if (["pending", "disabled"].includes(profileRow?.account_status)) {
    ...
    throw new Error(
      profileRow.account_status === "disabled"
...
export async function signUpWithEmailPassword({ email, password, profile }) {
  ...
  const requiresApproval = ["staff", "organizer"].includes(
...
export async function getCurrentAppUser() {
  ...
  // A missing session is the normal signed-out state during app startup.
  if (
...
export async function updateCurrentUserProfile(profile = {}) {
  ...
  const { data, error } = await supabase.auth.getUser();
  console.log("[auth.profile] auth.getUser done", {
  ...
  console.log("[auth.profile] profiles upsert done", {
  ...
  try {
    ...
    console.log("[auth.profile] auth.updateUser done", {
    ...
    console.log(
      "Profile metadata update timeout warning:",
      metadataTimeoutError?.message || metadataTimeoutError,
    );
  }
  ...
  try {
    ...
    console.log("[auth.profile] fetchProfile done", {
      durationMs: Date.now() - fetchProfileStartedAt,
    });
    console.log("[auth.profile] update completed", {
      durationMs: Date.now() - totalStartedAt,
    });
    return enrichUserWithAdminInfo(mapToAppUser(data.user, profileRow));
  } catch (fetchTimeoutError) {
    console.log(
      "Profile fetch timeout warning:",
      fetchTimeoutError?.message || fetchTimeoutError,
    );
    console.log("[auth.profile] update completed with fallback", {
      durationMs: Date.now() - totalStartedAt,
    });
    return {
      id: data.user.id,
      ...
      fullName:
        normalized.fullName || data.user.email?.split("@")?.[0] || "NSUK User",
  ...
  const result = supabase.auth.onAuthStateChange((event, session) => {
    ...
    setTimeout(async () => {
      try {
        ...
        if (
          !profileRow ||
          ["pending", "disabled"].includes(profileRow.account_status)
        ) {
          await supabase.auth.signOut();
          return;
        }
        const enrichedUser = await enrichUserWithAdminInfo(
          mapToAppUser(authUser, profileRow),
        );
        if (active && currentGeneration === generation)
          callback(event, enrichedUser, session);
      } catch (error) {
