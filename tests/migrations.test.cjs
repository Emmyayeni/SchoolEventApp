...
test('database migrations preserve records, enforce count visibility and allow catalog reads', async () => {
  ...
  try {
    ...
    await db.exec(`
      ...
      insert into events(title,category,description,event_date,venue,organizer,created_by,image_url)
      values ('Existing event', 'Custom category', 'Existing description', '2099-01-01', 'Hall', 'Organizer', '${organizer}',
        'https://images.unsplash.com/photo-1511578314322-379afb476865?auto=format&fit=crop&w=900&q=80');
    `);
    const migrations = ['202609130001_account_approval.sql', '202609140001_mobile_data.sql'];
